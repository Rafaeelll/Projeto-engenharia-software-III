...
import './styles/esqueci_senha.css'
...
export default function EsqueciSenha() {
    ...
    const [notif, setNotif] = React.useState({
      ...
      const API_PATH = '/usuarios/esqueci_senha'

      async function handleSubmit(event) {
        event.preventDefault()      // Impede o recarregamento da página
        setShowWaiting(true)        // Mostra o spinner de espera
        try {
          const result = await myfetch.post(`${API_PATH}`, {email})
          if (result) {
            // Exibe o snackbar de sucesso
            setNotif({
                show: true,
                message: 'Por favor, verifique o Token enviado no e-mail informado!',
                severity: 'success'
            })        
            setTimeout(()=>{
